#!/bin/bash
# Test remote MCP servers from smithery.ai with gmcpt list

GMCPT="/home/user/research/mcplist/gmcpt"
SERVERS_JSON="/home/user/research/mcplist/smithery_servers.json"
RESULTS_DIR="/home/user/research/mcplist/results_smithery_ai"
OUTPUT_FILE="/home/user/research/mcplist/smithery_ai.txt"

mkdir -p "$RESULTS_DIR"

TOTAL=$(python3 -c "import json; data=json.load(open('$SERVERS_JSON')); print(len(data))")
echo "Testing $TOTAL server endpoints from smithery.ai..."
...
# Write server list to temp file to avoid pipeline subshell issue
TMP_LIST=$(mktemp)
python3 -c "
import json
data = json.load(open('$SERVERS_JSON'))
for i, s in enumerate(data):
    name = s['name'].replace('|', '-')
    print(f\"{i}|{name}|{s['url']}|{s['transport']}\")
" > "$TMP_LIST"

# Run in parallel with concurrency limit of 20
PIDS=()
MAX_PARALLEL=20
...
wait
rm -f "$TMP_LIST"
echo "All tests complete. Compiling results..."
...
with open(output_file, 'w') as f:
    f.write("MCP Servers (smithery.ai) - Authentication & Capabilities Report\n")
    f.write("Generated by gmcpt list against smithery.ai\n")
    f.write("=" * 78 + "\n\n")
...
print(f"Successful: {gmcpt_success}, Auth required: {gmcpt_auth}, Blocked: {gmcpt_blocked}, Failed: {gmcpt_failed}, Timeout: {gmcpt_timeout}, Error: {gmcpt_error}")
PYEOF
